...
type CardState =
    ...
    | { status: 'ok'; items: DartItem[]; fsDiv: 'CFS' | 'OFS' };
...
function growthRate(cur: number | null, prev: number | null): number | null {
    if (cur === null || prev === null || prev === 0) return null;
    return ((cur - prev) / Math.abs(prev)) * 100;
}

/* ── 시사점 계산 ── */
function computeInsights(fetchedList: FetchedResult[]) {
    ...
    const companies = fetchedList.map(({ name, items, colorIdx }) => {
        if (!items) return { name, colorIdx, available: false as const, revGrowth: null, opinGrowth: null, netGrowth: null, opinMargin: null, netMargin: null, tags: [] as Tag[] };
        ...
        return { name, colorIdx, available: true as const, revGrowth, opinGrowth, netGrowth, opinMargin, netMargin, tags };
    });

    const withData = companies.filter(c => c.available);
    const opinMarginLeader = [...withData].filter(c => c.opinMargin !== null).sort((a, b) => b.opinMargin! - a.opinMargin!)[0] ?? null;
    const revGrowthLeader  = [...withData].filter(c => c.revGrowth !== null).sort((a, b) => b.revGrowth! - a.revGrowth!)[0] ?? null;

    return { companies, opinMarginLeader, revGrowthLeader, total: withData.length };
}

/* ── API 호출 ── */
async function callFinancialApi(corpCode: string, year: string, reprtCode: string, fsDiv: string) {
        ...
        <div className="tbl-wrap">
            ...
            <table>
                <thead>
                    <tr>
                        <th>항목</th>
                        <th>{first?.thstrm_nm ?? '당기'}<span className="th-period">당기</span></th>
                        <th>{first?.frmtrm_nm ?? '전기'}<span className="th-period">전기</span></th>
                    </tr>
                </thead>
                <tbody>
                    {ACCOUNTS.map(acc => {
...
function CompanyCard({ company, colorIdx, state }: { company: SelectedCompany; colorIdx: number; state: CardState }) {
    ...
    return (
        <div className="co-card">
            <div className="co-head" style={{ background: `linear-gradient(135deg, ${color}cc, ${color})` }}>
                ...
                <span className="fs-tag">{badge}</span>
            </div>
            <div className="co-body">
                {state.status === 'idle'    && <div className="state-box"><div className="state-text">조회 버튼을 눌러<br />데이터를 불러오세요.</div></div>}
                {state.status === 'loading' && <div className="state-box"><div className="spinner" /><div className="state-text">데이터를 불러오는 중...</div></div>}
                {state.status === 'err'     && <div className="state-box"><div className="state-icon state-error">⚠</div><div className="state-text state-error">{state.msg}</div></div>}
...
interface Tag { type: string; text: string; }

function MetricRow({ label, value }: { label: string; value: number | null }) {
    if (value === null) return <div className="metric-row"><span className="metric-label">{label}</span><span className="metric-na">-</span></div>;
    const isPos = value > 0;
    const isNeg = value < 0;
    return (
        <div className="metric-row">
            <span className="metric-label">{label}</span>
            <span className={`metric-value ${isPos ? 'up' : isNeg ? 'down' : 'neutral'}`}>
                {isPos ? '▲ ' : isNeg ? '▼ ' : ''}{isPos ? '+' : ''}{value.toFixed(1)}%
            </span>
        </div>
    );
}

function InsightsPanel({ insights }: { insights: ReturnType<typeof computeInsights> | null }) {
    return (
        <aside className="insights-panel">
            <div className="insights-header">⚡ 시사점</div>
            {!insights ? (
                <div className="insights-empty"><div className="state-text">조회 후 자동 생성됩니다.</div></div>
            ) : (
                <>
                    {insights.companies.map(co => (
                        <div key={co.name} className="insight-co">
                            <div className="insight-co-name" style={{ borderLeftColor: COMPANY_COLORS[co.colorIdx] ?? '#ccc' }}>
                                {co.name}
                            </div>
                            {!co.available ? (
                        ...
                        <div className="insights-summary">
                            ...
                            {insights.revGrowthLeader && (
                                <p className="summary-item">
                                    ...
                                    매출 성장률 <strong>{insights.revGrowthLeader.revGrowth! >= 0 ? '+' : ''}{insights.revGrowthLeader.revGrowth!.toFixed(1)}%</strong>로{' '}
                                    {insights.total > 1 ? `${insights.total}개사 중 ` : ''}성장률 최고
                                </p>
                            )}
                        </div>
                    )}
...
export default function Page() {
    ...
    function patch(corpCode: string, update: CardState) {
        setResults(prev => ({ ...prev, [corpCode]: update }));
    }

    const FATAL = new Set(['010', '011', '020', '100', '800']);

    async function fetchOne(co: SelectedCompany, colorIdx: number): Promise<FetchedResult> {
        try {
            // 연결재무제표 먼저 시도
            let data = await callFinancialApi(co.corp_code, year, reprtCode, 'CFS');
            const cfsItems = data.status === '000' ? extractIsItems(data.list ?? []) : null;
            if (cfsItems) {
                patch(co.corp_code, { status: 'ok', items: cfsItems, fsDiv: 'CFS' });
                return { name: co.corp_name, items: cfsItems, colorIdx };
            }

            if (FATAL.has(data.status)) {
                patch(co.corp_code, { status: 'err', msg: `API 오류 (${data.status}): ${data.message}` });
                return { name: co.corp_name, items: null, colorIdx };
            }

            // 개별재무제표로 폴백
            data = await callFinancialApi(co.corp_code, year, reprtCode, 'OFS');
            const ofsItems = data.status === '000' ? extractIsItems(data.list ?? []) : null;
            if (ofsItems) {
    ...
    async function handleSearch() {
        ...
        setBusy(true);
        setInsights(null);
        setResults(Object.fromEntries(selected.map(co => [co.corp_code, { status: 'loading' as const }])));
        const fetched = await Promise.all(selected.map((co, i) => fetchOne(co, i)));
        setInsights(computeInsights(fetched));
        setBusy(false);
    }

    const headerSub = selected.length > 0 ? selected.map(c => c.corp_name).join(' · ') : '회사를 선택해주세요';
            ...
            <main className="wrap">
                ...
                {selected.length === 0 ? (
                    ...
                    <div className="main-content">
                        <div className={`results-grid grid-${selected.length}`}>
                            {selected.map((co, i) => (
                                <CompanyCard
                                    key={co.corp_code}
                                    company={co}
